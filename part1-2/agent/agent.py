...
class Agent:
    def __init__(self,
        ...
        self.features_size = 3 + len(self.market_prices.columns[4:])
    ...
    def get_state(self, t):
        ...
        for counter in np.arange(0, self.window_size):
            maximum = np.max([self.market_prices['close'].iloc[t - counter],
                              self.market_prices['open'].iloc[t - counter]])
            minimum = np.min([self.market_prices['close'].iloc[t - counter],
                              self.market_prices['open'].iloc[t - counter]])
            candels = [((self.market_prices['close'].iloc[t - counter]
                         - self.market_prices['open'].iloc[t - counter])
                        / self.market_prices['open'].iloc[t - counter]) * 100,
                       ((self.market_prices['high'].iloc[t - counter]
                         - maximum) / maximum) * 100,
                       ((minimum - self.market_prices['low'].iloc[t - counter]) / minimum) * 100]
            candels = np.array(candels)
            signals = self.market_prices[self.market_prices.columns[4:]].iloc[1].values
            candel_signal = np.concatenate((candels, signals), axis=0)
            if counter == 0:
                state = np.zeros((candel_signal.shape[0], 1))
            state = np.column_stack((state, candel_signal.reshape(-1, 1)))
